...
def emit_gen_cpp(specs: dict[str, DelegateSpec]) -> str:
    parts = ['#include "LuaDelegates.gen.hpp"', "", "namespace luax {"]
    for spec in specs.values():
...
def main() -> None:
    ...
    SPECS_OUT.write_text(emit_specs_py(specs), encoding="utf-8")
    GEN_HPP.write_text(emit_gen_hpp(specs), encoding="utf-8")
    GEN_CPP.write_text(emit_gen_cpp(specs), encoding="utf-8")
