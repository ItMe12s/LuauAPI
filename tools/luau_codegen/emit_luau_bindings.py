...
def _collect_plan(root: Root, target_platform: str) -> tuple[
    List[Class],
    Dict[str, Class],
    List[tuple[str, str, str]],
    Set[str],
    dict[str, dict[str, list[Method]]],
    dict[str, List[tuple[Class, Method]]],
    dict[str, int],
    List[Class],
]:
...
def plan_outputs(root: Root, target_platform: str = "win") -> List[str]:
    *_, emitted_classes = _collect_plan(root, target_platform)
    outputs = ["bindings_internal.hpp", "bindings_common.cpp"]
    outputs.extend(_binding_filename(cls.name) for cls in emitted_classes)
    return outputs
...
def _emit_class_file(
    ...
    gen_ns = _gen_ns(cls)
    out = [_file_preamble(), '#include "bindings_internal.hpp"\n\n']
...
def emit(
    ...
    (
        ...
        depths,
        emitted_classes,
    ) = _collect_plan(root, target_platform)

    files: dict[str, str] = {
        "bindings_internal.hpp": _emit_internal_hpp(),
        "bindings_common.cpp": _emit_common_file(emitted_classes),
    }

    for cls in emitted_classes:
        files[_binding_filename(cls.name)] = _emit_class_file(
            cls,
            supported_by_class[cls.name],
            hook_targets_by_class[cls.name],
            objects,
            skipped_classes,
            depths[cls.name],
            target_platform,
