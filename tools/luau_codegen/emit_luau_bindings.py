...
def _collect_platform_plan(root: Root, target_platform: str) -> EmitPlan:
...
def _collect_plan(root: Root, target_platform: str) -> EmitPlan:
    raw_plans = {
        platform: _collect_platform_plan(root, platform)
        for platform in INTERSECTION_PLATFORMS
    }
    if target_platform in raw_plans:
        plan = raw_plans[target_platform]
    else:
        plan = _collect_platform_plan(root, target_platform)
...
def collect_platform_plan(root: Root, target_platform: str = "win") -> EmitPlan:
    return _collect_platform_plan(root, target_platform)
