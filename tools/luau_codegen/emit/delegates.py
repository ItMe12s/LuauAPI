...
from luau_codegen.model.delegate_specs import (
    DelegateCatalog,
    DelegateMethodSpec as ModelDelegateMethodSpec,
    DelegateSpec as ModelDelegateSpec,
)
...
def build_delegate_catalog(
    specs: dict[str, CppDelegateSpec],
    ctx: CodegenContext,
) -> DelegateCatalog:
    out: dict[str, ModelDelegateSpec] = {}
    for cxx, spec in sorted(specs.items()):
        methods = []
        for m in spec.methods:
            if not cpp_emit_supported(spec, m, ctx):
                continue
            rl = lua_for(m.ret, ctx)
            args_lua = _lua_arg_tuple([lua_for(a[0], ctx) for a in method_args(spec, m)])
            if rl is None or args_lua is None:
                continue
            methods.append(ModelDelegateMethodSpec(m.name, rl, args_lua))
        if not methods:
            continue
        out[cxx] = ModelDelegateSpec(
            cxx_type=cxx,
    ...
    return DelegateCatalog.from_specs(out)
...
def parse_delegate_methods(
    bindings_dir: Path | str,
    ctx: CodegenContext,
) -> dict[str, list[CppDelegateMethod]]:
    ...
    return parse_delegate_classes(
        ...
        ctx=ctx,
        norm=strip_ref,
        delegate_method_cls=CppDelegateMethod,
    )


def collect_delegate_specs(
    bindings_dir: Path | str, ctx: CodegenContext
) -> dict[str, CppDelegateSpec]:
    from luau_codegen.parse.broma_delegates import scan_delegate_bindings

    bro_dir = Path(bindings_dir)
    broma, ptrs = scan_delegate_bindings(
        bro_dir,
        ctx=ctx,
        norm=strip_ref,
        delegate_method_cls=CppDelegateMethod,
    ...
    for ptr in sorted(ptrs):
        ...
        for k in keys:
            if k in COCOS_DELEGATES:
                methods = [CppDelegateMethod(n, r, a) for n, r, a in COCOS_DELEGATES[k]]
                cxx = k
                break
            if k in broma:
                methods = broma[k]
        ...
        if not methods:
            continue
        specs[cxx] = CppDelegateSpec(cxx, short, f"Lua{short}", methods)
    return specs


def method_args(spec: CppDelegateSpec, m: CppDelegateMethod) -> list[tuple[str, str]]:
...
def should_emit_method(spec: CppDelegateSpec, m: CppDelegateMethod, ctx: CodegenContext) -> bool:
    ...
    return lua_for(m.ret, ctx) is not None and all(lua_for(a[0], ctx) for a in args)


def cpp_emit_supported(spec: CppDelegateSpec, m: CppDelegateMethod, ctx: CodegenContext) -> bool:
    if not should_emit_method(spec, m, ctx):
        return False
    ...
    if nret in ("gd::string", "char const*", "const char*", "std::string"):
        return True
    if nret.endswith("*"):
        return lua_for(m.ret, ctx) is not None
    return False
...
def emit_override(spec: CppDelegateSpec, m: CppDelegateMethod, ctx: CodegenContext) -> str:
    if not cpp_emit_supported(spec, m, ctx):
        return ""
...
def emit_delegate_cpp(specs: dict[str, CppDelegateSpec], ctx: CodegenContext) -> str:
    parts = [
    ...
    for spec in specs.values():
        if not any(cpp_emit_supported(spec, m, ctx) for m in spec.methods):
            continue
...
def _warn_unsupported_emitters(specs: dict[str, CppDelegateSpec], ctx: CodegenContext) -> None:
    for spec in specs.values():
        for m in spec.methods:
            if should_emit_method(spec, m, ctx) and not cpp_emit_supported(spec, m, ctx):
                print(
...
def emit_delegate_artifacts(
    ...
    *,
    ...
    specs: dict[str, CppDelegateSpec] | None = None,
) -> DelegateCatalog:
    if specs is None:
        specs = collect_delegate_specs(bindings_dir, ctx)
    _warn_unsupported_emitters(specs, ctx)
    if gen_out is not None:
        gen_root = Path(gen_out)
        gen_files = {
            DELEGATE_GEN_REL_PATHS[0]: emit_delegate_hpp(specs, ctx),
            DELEGATE_GEN_REL_PATHS[1]: emit_delegate_cpp(specs, ctx),
        }
        for rel, content in gen_files.items():
            _write_if_changed(str(gen_root / rel), content)
    return build_delegate_catalog(specs, ctx)
