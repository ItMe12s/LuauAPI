...
def emit(
    ...
    for cls in classes:
        ...
        class_lines, has_members = _emit_class(
            cls,
            grouped,
            objects,
            skipped_classes,
        ...
        chunk = "".join(class_lines)
        if not has_members:
            empty_chunk_names.add(cls.name)
    ...
    orphan_chunk_names = {
        name
        for name, _ in cocos_chunks + gd_chunks
        if name not in base_names and name in empty_chunk_names
    }
    ...
    cocos_factory_text = "".join(
        _emit_factories(
            cocos_factories, objects, cocos_namespace, ctx=plan.ctx, analysis=plan.type_analysis
        )
    )
    ...
    geode_factory_text = "".join(
        _emit_factory_records(geode_factories, objects, ctx=plan.ctx, analysis=plan.type_analysis)
    )

    function_tree = _build_function_tree(plan.supported_free_functions, objects, manual_fields)
    function_field_lines = _emit_function_tree(
        function_tree, objects, 1, ctx=plan.ctx, analysis=plan.type_analysis
    )
    free_function_refs = _refs_from_functions(
        plan.supported_free_functions, objects, ctx=plan.ctx, analysis=plan.type_analysis
    )
    ...
    refs |= _factory_object_refs(
        cocos_factories, objects, ctx=plan.ctx, analysis=plan.type_analysis
    )
    refs |= _factory_object_refs(gd_factories, objects, ctx=plan.ctx, analysis=plan.type_analysis)
    refs |= _factory_object_refs(
        geode_factories, objects, ctx=plan.ctx, analysis=plan.type_analysis
    )
    ...
    orphan_names = {
        name
        for name in refs
        if name not in defined and name in objects and name not in plan.ctx.value_types.stub_body
    } | orphan_chunk_names

    lines = _header("Geode type stubs")
    lines.append(_emit_generated_support_stub_block())

    delegate_block = _emit_delegate_stub_block(plan.ctx.delegates)
