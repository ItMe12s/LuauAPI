...
def emit_internal_hpp() -> str:
    return (
        ...
        "    auto* runtime = luax::Runtime::fromState(L);\n"
        "    if (!runtime) {\n"
        '        geode::log::error("luau hook [{}] override rejected: runtime unavailable", targetId);\n'
        "        return false;\n"
        "    }\n"
        ...
        "    if (runtime->protectedCallWithTraceback(L, 1, 0, targetId, {.record = false}).isErr()) {\n"
        "        lua_settop(L, top);\n"
        "        return false;\n"
        "    }\n"
        "    lua_settop(L, top);\n"
        ...
        "    void* ctx;\n"
        "};\n\n"
        "inline int luaapi_apply_noop(lua_State* L) {\n"
        "    (void)L;\n"
        "    return 0;\n"
        "}\n\n"
        "inline void adoptHookThread() {\n"
        "    if (!luax::Runtime::isMainThread()) {\n"
        "        luax::Runtime::setMainThreadId(std::this_thread::get_id());\n"
        "    }\n"
        "}\n\n"
        "inline bool runLuaPreHooks(char const* targetId, int nargs, void (*pushFn)(lua_State*, void*), void* pushCtx, HookApply applyArgs, HookApply applySkip) {\n"
        "    adoptHookThread();\n"
        ...
        "        if (runtime->protectedCall(L, nargs, 1, targetId, luax::kHookScriptDeadlineMs, {luax::diag::BoundaryKind::HookBefore, callback->id}).isOk()) {\n"
        "            if (lua_istable(L, -1)) {\n"
        ...
        "                    if (!applyHookOverride(L, -1, applySkip.fn, applySkip.ctx, targetId)) {\n"
        "                        lua_pop(L, 1);\n"
        "                        lua_settop(L, top);\n"
        "                        continue;\n"
        "                    }\n"
        ...
        "                if (!lua_isnil(L, -1)) applyHookOverride(L, -1, applyArgs.fn, applyArgs.ctx, targetId);\n"
        "                lua_pop(L, 1);\n"
        ...
        "    return false;\n"
        "}\n\n"
        "inline void runLuaPostHooks(char const* targetId, int nargs, void (*pushFn)(lua_State*, void*), void* pushCtx, HookApply applyReturn) {\n"
        "    adoptHookThread();\n"
        "    auto it = hookStates().find(targetId);\n"
        ...
        "        pushFn(L, pushCtx);\n"
        "        luax::Runtime::ResourcesRootScope rootScope(*runtime, callback->ref.resourcesRoot());\n"
        "        if (runtime->protectedCall(L, nargs, 1, targetId, luax::kHookScriptDeadlineMs, {luax::diag::BoundaryKind::HookAfter, callback->id}).isOk()) {\n"
        "            if (!lua_isnil(L, -1)) {\n"
        "                if (!applyHookOverride(L, -1, applyReturn.fn, applyReturn.ctx, targetId)) {\n"
        "                    lua_settop(L, top);\n"
        "                    continue;\n"
        "                }\n"
        "            }\n"
...
def emit_hook_support() -> str:
    ...
    bool stateHasLiveCallbacks(LuaHookState const& state) {
        return geode::utils::ranges::contains(
            state.callbacks,
            [](auto const& callback) { return callback && !callback->removed; }
        );
    }
