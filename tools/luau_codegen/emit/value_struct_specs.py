...
from luau_codegen.convert.type_primitives import COMPOSITE_KINDS, iter_type_tree
...
def collect_value_struct_specs(root: "Root") -> tuple[ValueTypeSpec, ...]:
    from luau_codegen.model.value_struct_gate import VALUE_STRUCT_OPT_IN

    base_ctx = root.codegen_ctx or CodegenContext.static()
    objects = {c.name: c for c in root.classes}
    opt_in = set(VALUE_STRUCT_OPT_IN)

    provisional_specs = tuple(ValueTypeSpec(lua_name=name, cxx_type=name) for name in opt_in)
    ctx = base_ctx.with_value_specs(provisional_specs)
