...
def emit_internal_hpp() -> str:
    return (
        ...
        "                    bool ok = applySkip(L, -1);\n"
        "                    lua_pop(L, 1);\n"
        '                    if (!ok) geode::log::warn("luau hook [{}] returned invalid skip value", targetId);\n'
        "                    if (!ok) {\n"
        "                        lua_settop(L, top);\n"
        "                        continue;\n"
        "                    }\n"
        ...
        '                    geode::log::warn("luau hook [{}] returned invalid args override", targetId);\n'
        "                }\n"
        "                lua_pop(L, 1);\n"
        "            } else if (!lua_isnil(L, -1)) {\n"
        '                geode::log::warn("luau hook [{}] returned invalid before result", targetId);\n'
        "            }\n"
        "        }\n"
        "        lua_settop(L, top);\n"
        "    }\n"
