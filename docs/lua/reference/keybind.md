...
This page lists `geode.Keybind`. It reads and writes key combinations.

A keybind is a plain table:
...
`fromString` returns `nil` and an error message on a bad string, so you can handle it
without `pcall`.
...
Parses a string such as `"Ctrl + A"` into a keybind. Returns the keybind, or `nil` and an
error message.
