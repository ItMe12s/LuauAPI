...
This page lists `geode.VersionInfo`. It parses and compares version strings.

It uses a small part of semver. A version looks like `v1.2.3` or `v1.2.3-beta.1`. The
functions take strings, so you do not build a version by hand.

The parse and compare functions return `nil` and an error message on a bad version, so you
can handle them without `pcall`.
...
Tests a version against a constraint such as `>=v1.2.0`. Returns `true` or `false`, or
