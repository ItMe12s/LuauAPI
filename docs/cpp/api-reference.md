...
Runs inline source, and must run on the main thread. `chunkName` names the chunk in logs and errors. The source must be `4 MiB` or smaller. It returns `Ok` or `Err`.
