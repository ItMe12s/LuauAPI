...
- `cocos2d::ccCArray*` on `CCKeyboardDispatcher.m_pHandlersToAdd` -> `{ CCKeyboardHandler? }`

Same pattern for `CCKeypadHandler`, `CCMouseHandler`,
and `CCTouchHandler` on the matching dispatcher `m_pHandlersToAdd` / `m_pHandlersToRemove` fields listed in the allowlist.
...
3. `CCArray.data` and other unlisted `ccCArray*` fields remain `-- skipped`
