...
The binding exposes the `task` and `time` libraries, and the scheduler stores the callbacks and fires them when due.

## The binding

`TaskBinding.cpp` builds the `task` and `time` global tables and registers them with a binding.
It also sets the time origin, registers the handle metatable, arms the tick,
...
The handle returned to Lua is a small userdata that holds a task id and provides a `cancel` method.
...
`advance(dt, L)` runs each frame. It lowers each task timer by the frame delta and fires the tasks that are due.
To fire a task, it pushes the callback, sets the resources root scope, and calls the runtime protected call with the callback budget.
A repeating task is rescheduled, while a one shot or deferred task is marked cancelled. Cancelled tasks are then removed.
...
The scheduler holds at most `4096` tasks. The binding checks capacity before adding and raises an error when full.
