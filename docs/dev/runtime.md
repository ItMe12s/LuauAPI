...
It holds up to `512` entries and `64 MiB` total (`kMaxBytecodeCacheEntries`, `kMaxBytecodeCacheBytes` in `Config.hpp`).
Before insert, `trimBytecodeCacheForInsert` evicts using one combined check over those byte
and entry limits plus runtime memory usage (`bytecodeCacheInsertNeedsEviction` in `BytecodeCacheAccounting.hpp`).
