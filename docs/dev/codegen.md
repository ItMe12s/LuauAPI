...
- Luau type stubs into `types/`, one `.d.luau` file per class plus a per-namespace factories file, the `geode.d.luau` root, and a `luau-lsp.json` listing them in load order. Splitting per class keeps each file small enough to avoid luau-lsp's "Code is too complex to typecheck" error. These give editors autocomplete and type checks.
