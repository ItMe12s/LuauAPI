...
`BytecodeCacheKey.hpp` builds the cache key from the file path, size, modify time, and a content hash.
The runtime uses this key in its least recently used cache.
