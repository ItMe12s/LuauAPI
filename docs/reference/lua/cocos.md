...
Node helpers and hex string functions come from codegen over `Geode/utils/cocos.hpp`.
...
`applyH`/`applyHue`, `applyS`/`applySaturation`, `applyV`/`applyValue`, and `applyB`/`applyBrightness`.
