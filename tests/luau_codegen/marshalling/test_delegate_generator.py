...
from test_support import DELEGATE_FIXTURE_DIR, DELEGATE_SPECS, fixture_codegen_context
...
class DelegateGeneratorTests(unittest.TestCase):
    ...
    def test_delegate_spec_cpp_parity(self) -> None:
        specs = collect_delegate_specs(DELEGATE_FIXTURE_DIR, self.ctx)
        hpp = emit_delegate_hpp(specs, self.ctx)
    ...
    def test_generated_delegate_classes_declare_destructor(self) -> None:
        ...
        for spec in specs.values():
            if not any(cpp_emit_supported(spec, m, self.ctx) for m in spec.methods):
                continue
    ...
    def test_get_song_file_name_emits_invoke_table_string(self) -> None:
        spec = CppDelegateSpec(
            ...
            methods=[CppDelegateMethod("getSongFileName", "gd::string", [])],
        )
    ...
    def test_object_pointer_return_emits_invoke_table_object(self) -> None:
        spec = CppDelegateSpec(
            ...
            lua_name="CustomSongDelegate",
            cpp_class="LuaCustomSongDelegate",
            methods=[CppDelegateMethod("getLevelSettings", "LevelSettingsObject*", [])],
        )
        text = emit_override(spec, spec.methods[0], self.ctx)
    ...
    def test_table_view_delegate_emits_ccindexpath_reference(self) -> None:
        ...
        text = emit_override(spec, method, self.ctx)
        self.assertIn(
    ...
    def test_register_unregister_cast_use_qualified_cxx_type(self) -> None:
        ...
        spec = specs[cxx]
        hpp = emit_delegate_hpp({cxx: spec}, self.ctx)
        cpp = emit_delegate_cpp({cxx: spec}, self.ctx)
    ...
    def test_float_return_emits_invoke_table_value(self) -> None:
        ...
        self.assertTrue(cpp_emit_supported(spec, spec.methods[0], self.ctx))
        text = emit_override(spec, spec.methods[0], self.ctx)
