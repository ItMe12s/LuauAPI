...
class $modify(ImGuiCocosCCMouseDispatcher, CCMouseDispatcher) {
    bool dispatchScrollMSG(float y, float x) {
        if (!ImGuiCocos::get().isInitialized()) return CCMouseDispatcher::dispatchScrollMSG(y, x);
...
class $modify(ImGuiCocosCCIMEDispatcher, CCIMEDispatcher) {
    void dispatchInsertText(char const* text, int len, enumKeyCodes keys) {
        ...
        switch (keys) {
            case KEY_Left:
            case KEY_Right: return; // cocos sends 'a' text alongside the keycode
            default: break;
        }
    ...
    void dispatchDeleteBackward() {
        if (!ImGuiCocos::get().isInitialized()) return CCIMEDispatcher::dispatchDeleteBackward();
...
$execute {
    KeyboardInputEvent()
        .listen([](auto& evt) {
            if (!ImGuiCocos::get().isInitialized()) return ListenerResult::Propagate;

            bool const shouldEatInput = ImGui::GetIO().WantCaptureKeyboard;
            bool const isDown = evt.action != KeyboardInputData::Action::Release;
            if (shouldEatInput || !isDown) {
                auto const imKey = cocosToImGuiKey(evt.key);
                if (imKey != ImGuiKey_None) {
                    ImGui::GetIO().AddKeyEvent(imKey, isDown);

                    // handle pasting
                    if (
                        isDown && evt.key == KEY_V
#ifdef GEODE_IS_MACOS
...
class $modify(ImGuiCocosCCTouchDispatcher, CCTouchDispatcher) {
    void touches(CCSet* touches, CCEvent* event, unsigned int type) {
        ...
        if (geode::cocos::getMousePos().isZero()) {
            ...
            auto const pos = ImGuiCocos::cocosToFrame(touch->getLocation());
